# -*- coding: utf-8 -*-
"""Deprecated wrapper name: build lightweight MLIR-derived DES artifact summaries.

Use scripts/build_artifact_des_summary.py in V3.3 docs. The input is a compiler/modeling artifact generated from MLIR, not real profiling data.
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Deprecated: build MLIR-derived artifact DES summary JSON files.")
    ap.add_argument("--des-profile", default=None, help="Deprecated name for a raw MLIR-derived artifact DES graph JSON containing an operations array.")
    ap.add_argument("--mlir", default="", help="Optional MLIR/HIVM file associated with the artifact DES graph.")
    ...
    if not args.des_profile or not args.output:
        raise SystemExit("Either use --manifest --output-dir, or use --des-profile --output. Prefer scripts/build_artifact_des_summary.py --artifact-des-graph in V3.3 docs.")
    summary = summarize_des_trace(args.des_profile, mlir_file=args.mlir, sample_id=args.sample_id)
