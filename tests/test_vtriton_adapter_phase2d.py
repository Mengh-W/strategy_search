...
def _strategy():
    return {
        "strategy_id": "phase2d_cpp_bridge_test",
...
def test_phase2e_cpp_bridge_compiles_and_rewrites_barrier_and_cv_sync(tmp_path):
    ...
    non_comment = "\n".join(line for line in out.splitlines() if not line.lstrip().startswith("//"))
    assert 'hivm.hir.barrier {mode = "ALL"}' not in non_comment
    assert "hivm.hir.set_flag[<PIPE_MTE2>, <PIPE_M>, <EVENT_ID0>]" in out
    assert "hivm.hir.wait_flag[<PIPE_MTE2>, <PIPE_M>, <EVENT_ID0>]" in out
    assert "hivm.hir.set_flag[<PIPE_FIX>, <PIPE_V>, <EVENT_ID1>]" in out
    assert "hivm.hir.wait_flag[<PIPE_FIX>, <PIPE_V>, <EVENT_ID1>]" in out
    assert out.index("hivm.hir.wait_flag[<PIPE_FIX>, <PIPE_V>, <EVENT_ID1>]") < out.index("hivm.hir.vexp")
    ...
    assert report["change_counts"]["replace_barrier_all_with_directional_sync"] == 1
    assert report["change_counts"]["insert_sync_before_first_vector_op"] == 1
