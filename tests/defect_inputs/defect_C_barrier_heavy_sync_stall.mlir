// 缺陷注入样例：defect_C_barrier_heavy_sync_stall
// 目的：用于检验策略搜索器是否能识别低效结构，并给出更合理的候选策略。
// 明显缺陷说明：
// 1) tile 和数据类型相对合理，但同步过于粗粒度。
// 2) 循环内部多次全局同步，理论上会造成不必要的等待。
// 3) 该样例用于测试 SyncPlan 是否会向更细粒度同步策略移动。
// 注意：这些说明刻意避免使用会被当前解析器误识别为实际结构的英文触发词。

module {
  func.func @defect_C_barrier_heavy_sync_stall(
