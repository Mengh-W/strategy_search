module {
  ...
  func.func @fa(%Q_gm : memref<64x128xf16, #hivm.address_space<gm>>, %K_gm : memref<1024x128xf16, #hivm.address_space<gm>>, %V_gm : memref<1024x128xf16, #hivm.address_space<gm>>, %O_gm : memref<64x128xf16, #hivm.address_space<gm>>) {
    ...
    scf.for %m_outer = %c0 to %cM step %c32 {
      scf.for %k_outer = %c0 to %cK step %c128 {
        scf.for %n_outer = %c0 to %cN step %c64 {
          %Q_gm_tile_v63_0 = memref.subview %Q_gm[%m_outer, %k_outer] [32, 128] [1, 1] : memref<64x128xf16, #hivm.address_space<gm>> to memref<32x128xf16, #hivm.address_space<gm>>
          ...
          scf.for %j = %c0 to %cE step %cB {   // @trip=10
            ...
            hivm.hir.nd2nz ins(%k_ub : memref<64x128xf16, #hivm.address_space<ub>>) outs(%k_l1_pong : memref<64x128xf16, #hivm.address_space<cbuf>>)
    ...
    hivm.hir.wait_flag {pipe="MTE3", event="EVENT_ID_CVP_C2S_0", producer_pipe="V", consumer_pipe="MTE3"}
          ...
          %O_gm_tile_v63_3 = memref.subview %O_gm[%m_outer, %c0] [32, 128] [1, 1] : memref<64x128xf16, #hivm.address_space<gm>> to memref<32x128xf16, #hivm.address_space<gm>>
